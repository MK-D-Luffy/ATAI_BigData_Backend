package com.atai.ucenter.service.impl;
...
import com.atai.ucenter.mapper.UcenterBasicMapper;
 ...
 */
@Service
public class UcenterBasicServiceImpl extends ServiceImpl<UcenterBasicMapper, UcenterBasic> implements UcenterBasicService {

    @Autowired
    private RedisTemplate<String, String> redisTemplate;
    @Autowired
    private UcenterResumeService ucenterResumeService;
    ...
    public String login(LoginVo loginVo) {
        ...
        //校验手机号或邮箱是否存在
        QueryWrapper<UcenterBasic> queryWrapper = new QueryWrapper<>();
        ...
        //获取用户信息
        UcenterBasic member = baseMapper.selectOne(queryWrapper);
        if (member == null) {
...
//                redisTemplate.opsForValue().set(mobileOrEmail + "LoginFailedCount", loginFailedCount + "");
    ...
    public void register(RegisterVo registerVo) {
        ...
        //数据添加数据库中
        UcenterBasic member = new UcenterBasic();
        ...
        UcenterResume resume = new UcenterResume();
        resume.setId(id);
        ucenterResumeService.save(resume);
    }
    ...
    public Boolean checkNickname(String nickname) {
        QueryWrapper<UcenterBasic> wrapper = new QueryWrapper<>();
    ...
    public void changePwd(ChangePwdVo changePwdVo) {
        ...
        String mobileOrEmail = "";
        QueryWrapper<UcenterBasic> wrapper = new QueryWrapper<>();
        UcenterBasic UcenterBasic = baseMapper.selectById(id);

        //非空判断
        if (!StringUtils.isEmpty(code)) {
            if (!StringUtils.isEmpty(email)) {
                mobileOrEmail = email;
                if (!email.equals(UcenterBasic.getEmail())) {
            ...
            if (!StringUtils.isEmpty(mobile)) {
                mobileOrEmail = mobile;
                if (!mobile.equals(UcenterBasic.getMobile())) {
        ...
        UcenterBasic member = baseMapper.selectOne(wrapper);
        member.setPassword(MD5.encrypt(password));//密码需要进行MD5加密
    ...
    public boolean changeMobileOrEmail(ChangeMobileOrEmailVo changeMobileOrEmailVo, String id) {
        ...
        UcenterBasic UcenterBasic = baseMapper.selectById(id);
        if (!StringUtils.isEmpty(email)) {
            UcenterBasic.setEmail(email);
        }
        if (!StringUtils.isEmpty(mobile)) {
            UcenterBasic.setMobile(mobile);
        }
        return baseMapper.updateById(UcenterBasic) > 0;
    }
    ...
    @Override
    public Map<String, Object> getUserListPage(Page<UcenterBasic> userPage) {
        //把分页数据封装到userPage对象里去
        baseMapper.selectPage(userPage, null);

        List<UcenterBasic> records = userPage.getRecords();
